...
(async () => {
  ...
  await fs.writeFile("opioid-analgesics-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("opioid-analgesics-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
